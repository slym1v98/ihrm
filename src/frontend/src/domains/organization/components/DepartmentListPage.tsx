       ...
       <TableRow key={dept.id}>
        ...
        <TableCell className="text-right">
         <div className="flex justify-end gap-1">
          <Button variant="ghost"  title="Sửa" onClick={() => openEdit(dept)}>
           <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost"  title="Di chuyển" onClick={() => openMove(dept)}>
   ...
   <Dialog open={!!confirm} onOpenChange={(open) => !open && setConfirm(null)}>
    <DialogContent>
     ...
     <DialogFooter>
      ...
      <Button  variant={confirm?.action === 'deactivate' ? 'destructive' : 'primary'}
