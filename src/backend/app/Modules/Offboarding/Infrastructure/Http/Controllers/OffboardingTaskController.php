...
class OffboardingTaskController extends Controller
{
    ...
    {
        try {
            $this->completeHandler->handle(new CompleteTaskCommand($id, $request->input('proof_file_object_id')));
    ...
    {
        try {
            $this->waiveHandler->handle(new WaiveTaskCommand($id, $request->input('reason')));

            return response()->json(['message' => 'Waived']);
        } catch (\Throwable $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        }
    }
