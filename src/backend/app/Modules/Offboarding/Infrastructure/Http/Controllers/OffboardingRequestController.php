...
class OffboardingRequestController extends Controller
{
    ...
    public function show(string $id): JsonResponse
    {
        ...
        if (!$r) { throw new OffboardingRequestNotFoundException($id); }
        return response()->json(['data' => ['id' => $r->getId()->value, 'status' => $r->getStatus()->value]]);
    }

    public function submit(string $id): JsonResponse
    {
        try { $this->submitHandler->handle(new SubmitOffboardingRequestCommand($id)); return response()->json(['message' => 'Submitted']); }
    ...
    public function approve(Request $request, string $id): JsonResponse
    {
        try {
            ...
            $this->approveHandler->handle($cmd); return response()->json(['message' => 'Approved']);
        } catch (\Throwable $e) { return response()->json(['message' => $e->getMessage()], 422); }
    }

    public function reject(Request $request, string $id): JsonResponse
    {
        try { $this->rejectHandler->handle(new RejectOffboardingRequestCommand($id, $request->input('reason'))); return response()->json(['message' => 'Rejected']); }
        catch (\Throwable $e) { return response()->json(['message' => $e->getMessage()], 422); }
    }
}
