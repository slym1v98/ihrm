...
class OffboardingPlanController extends Controller
{
    ...
    public function show(string $id): JsonResponse
    {
        ...
        if (! $plan) {
            throw new OffboardingPlanNotFoundException($id);
        }
    ...
    {
        try {
            $this->activateHandler->handle(new ActivateOffboardingPlanCommand($id));
    ...
    {
        try {
            $this->completeHandler->handle(new CompleteOffboardingPlanCommand($id));
    ...
    {
        try {
            $this->clearanceHandler->handle(new CompleteFinalClearanceCommand($id, $request->user()?->id ?? $request->input('cleared_by', 'system'), $request->input('payroll_notes')));
