...
class EloquentCompetencyTemplateRepository implements CompetencyTemplateRepositoryInterface
{
    ...
    {
        $m = CompetencyTemplateModel::find($id->value);
    ...
    {
        $m = CompetencyTemplateModel::where('code', $code)->first();
    ...
    {
        return CompetencyTemplateModel::orderBy('code')->get()->map(fn ($m) => $this->toDomain($m))->toArray();
    }

    public function save(CompetencyTemplate $template): void
    {
        CompetencyTemplateModel::updateOrCreate(['id' => $template->getId()->value], ['code' => $template->getCode(), 'name' => $template->getName(), 'rules' => $template->getRules(), 'active' => $template->isActive()]);
    }

    public function delete(CompetencyTemplateId $id): void
    {
        CompetencyTemplateModel::where('id', $id->value)->delete();
    }

    private function toDomain(CompetencyTemplateModel $m): CompetencyTemplate
    {
        return CompetencyTemplate::reconstitute(CompetencyTemplateId::fromString($m->id), $m->code, $m->name, $m->rules ?? [], (bool) $m->active);
    }
}
