...
{
    public function findById(GoalId $id): ?Goal { $m = GoalModel::find($id->value); return $m ? $this->toDomain($m) : null; }
    public function findByCycleId(string $cycleId): array { return GoalModel::where('cycle_id', $cycleId)->orderBy('sort_order')->get()->map(fn($m) => $this->toDomain($m))->toArray(); }
    public function findByEmployeeId(string $employeeId): array { return GoalModel::where('employee_id', $employeeId)->get()->map(fn($m) => $this->toDomain($m))->toArray(); }
