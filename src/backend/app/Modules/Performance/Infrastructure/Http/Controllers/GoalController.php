...
class GoalController extends Controller
{
    ...
    {
        try {
            $this->updateHandler->handle(new UpdateGoalCommand($id, $r->input('title'), $r->input('description'), (float) $r->input('weight', 1.0), $r->input('target_value')));
    ...
    {
        try {
            $this->completeHandler->handle(new CompleteGoalCommand($id, $r->input('actual_value')));
