...
class PerformanceReviewController extends Controller
{
    ...
    { try { $this->selfHandler->handle(new SubmitSelfReviewCommand($id, $r->input('assessment', []))); return response()->json(['message' => 'Self assessment submitted']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }

    public function submitManager(Request $r, string $id): JsonResponse
    { try { $this->managerHandler->handle(new SubmitManagerReviewCommand($id, $r->input('assessment', []))); return response()->json(['message' => 'Manager assessment submitted']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }

    public function submitHr(Request $r, string $id): JsonResponse
    { try { $this->hrHandler->handle(new SubmitHrReviewCommand($id, $r->input('assessment', []))); return response()->json(['message' => 'HR assessment submitted']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }

    public function finalize(Request $r, string $id): JsonResponse
    { try { $this->finalizeHandler->handle(new FinalizeReviewCommand($id, $r->input('final_score'))); return response()->json(['message' => 'Review finalized']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }
}
