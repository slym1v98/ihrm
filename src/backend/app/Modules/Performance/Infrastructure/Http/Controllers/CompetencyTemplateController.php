...
class CompetencyTemplateController extends Controller
{
    ...
    { try { $this->updateHandler->handle(new UpdateTemplateCommand($id, $r->input('code'), $r->input('name'), $r->input('rules', []))); return response()->json(['message' => 'Updated']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }

    public function destroy(string $id): JsonResponse
    { try { $this->templateRepo->delete(CompetencyTemplateId::fromString($id)); return response()->json(['message' => 'Deleted']); } catch (\Exception $e) { return response()->json(['error' => $e->getMessage()], 422); } }
}
