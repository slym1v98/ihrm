...
use App\Modules\Recruitment\Domain\Exceptions\CandidateConversionException;
...
class ConvertCandidateToEmployeeHandler
{
    ...
    public function handle(ConvertCandidateToEmployeeCommand $cmd): string
    {
        ...
        if ($o->getStatus() !== OfferStatus::Accepted) {
            throw new CandidateConversionException('Offer must be accepted');
        }
        $c = $this->candidates->findById(new CandidateId($o->getCandidateId()));
        if (! $c) {
            throw new CandidateNotFoundException($o->getCandidateId());
        }
        if ($c->getEmployeeId() !== null) {
            throw new CandidateConversionException('Already converted');
        }
        $req = $this->reqs->findById(new RecruitmentRequisitionId($o->getRequisitionId()));
        if (! $req) {
            throw new CandidateConversionException('Requisition not found');
        }
        $empId = $this->conversion->convert($c, $req);
        $c->linkEmployee($empId);
        $this->candidates->save($c);
