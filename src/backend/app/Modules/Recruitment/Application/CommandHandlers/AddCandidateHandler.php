...
class AddCandidateHandler
{
    ...
    {
        if ($cmd->email && $this->repo->findByEmail($cmd->email)) {
            throw new DuplicateCandidateException('email:'.$cmd->email);
        }
        if ($cmd->phone && $this->repo->findByPhone($cmd->phone)) {
            throw new DuplicateCandidateException('phone:'.$cmd->phone);
        }
