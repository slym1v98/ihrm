...
use App\Modules\Recruitment\Application\CommandHandlers\{CreateOfferHandler, AcceptOfferHandler, RejectOfferHandler, ConvertCandidateToEmployeeHandler};
use App\Modules\Recruitment\Application\Commands\{CreateOfferCommand, AcceptOfferCommand, RejectOfferCommand, ConvertCandidateToEmployeeCommand};
...
class OfferController {
    ...
    public function convert(Request $r, string $id): JsonResponse {
        try { $empId=$this->convertHandler->handle(new ConvertCandidateToEmployeeCommand($id)); return response()->json(['data'=>['employee_id'=>$empId]]); }
