...
class EloquentInterviewRepository implements InterviewRepositoryInterface {
    ...
    public function findById(InterviewId $id): ?Interview { $r=$this->model->find($id->value()); return $r?self::toDomain($r):null; }
    public function list(?string $candidateId=null): array { $q=$this->model->orderByDesc('scheduled_at'); if($candidateId) $q->where('candidate_id',$candidateId); return $q->get()->map(fn($r)=>self::toDomain($r))->all(); }
