...
class EloquentCandidateRepository implements CandidateRepositoryInterface {
    ...
    public function findById(CandidateId $id): ?Candidate { $r=$this->model->find($id->value()); return $r?self::toDomain($r):null; }
    public function findByEmail(string $email): ?Candidate { $r=$this->model->where('email',$email)->first(); return $r?self::toDomain($r):null; }
    public function findByPhone(string $phone): ?Candidate { $r=$this->model->where('phone',$phone)->first(); return $r?self::toDomain($r):null; }
    public function list(?string $requisitionId=null): array { $q=$this->model->orderByDesc('created_at'); if($requisitionId) $q->where('requisition_id',$requisitionId); return $q->get()->map(fn($r)=>self::toDomain($r))->all(); }
