...
class TrainingEnrollment {
    ...
    public function recordAttendance(array $attendance): void {
        if ($this->status !== EnrollmentStatus::Enrolled && $this->status !== EnrollmentStatus::Completed) throw new InvalidEnrollmentStatusException($this->status->value, 'record attendance');
        $this->attendance = $attendance;
        $this->recordedEvents[] = new AttendanceRecorded($this->id->value);
    }
    public function complete(): void {
        if (!$this->status->canTransitionTo(EnrollmentStatus::Completed)) throw new InvalidEnrollmentStatusException($this->status->value, EnrollmentStatus::Completed->value);
        $this->status = EnrollmentStatus::Completed;
    }
    public function cancel(): void {
        if (!$this->status->canTransitionTo(EnrollmentStatus::Cancelled)) throw new InvalidEnrollmentStatusException($this->status->value, EnrollmentStatus::Cancelled->value);
        $this->status = EnrollmentStatus::Cancelled;
        $this->recordedEvents[] = new EnrollmentCancelled($this->id->value);
    }
    public function getId(): TrainingEnrollmentId { return $this->id; }
    ...
    public function getAttendance(): ?array { return $this->attendance; }
