...
class TrainingSession
{
    ...
    {
        if ($this->maxParticipants !== null && $currentCount >= $this->maxParticipants) {
    ...
    {
        if (! $this->status->canTransitionTo(SessionStatus::Active)) {
            throw new InvalidEnrollmentStatusException($this->status->value, SessionStatus::Active->value);
        } $this->status = SessionStatus::Active;
    }

    public function complete(): void
    {
        if (! $this->status->canTransitionTo(SessionStatus::Completed)) {
            throw new InvalidEnrollmentStatusException($this->status->value, SessionStatus::Completed->value);
        } $this->status = SessionStatus::Completed;
    }

    public function cancel(): void
    {
        if (! $this->status->canTransitionTo(SessionStatus::Cancelled)) {
            throw new InvalidEnrollmentStatusException($this->status->value, SessionStatus::Cancelled->value);
