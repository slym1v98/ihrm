...
use App\Modules\Training\Application\CommandHandlers\CancelEnrollmentHandler;
use App\Modules\Training\Application\CommandHandlers\CompleteEnrollmentHandler;
use App\Modules\Training\Application\CommandHandlers\EnrollEmployeeHandler;
use App\Modules\Training\Application\CommandHandlers\RecordAttendanceHandler;
...
use App\Modules\Training\Application\Commands\RecordAttendanceCommand;
use App\Modules\Training\Application\Queries\ListEnrollmentsQuery;
use App\Modules\Training\Application\QueryHandlers\ListEnrollmentsHandler;
...
class TrainingEnrollmentController extends Controller
{
    ...
    {
        $items = $this->listHandler->handle(new ListEnrollmentsQuery($sessionId));
    ...
    public function store(Request $r, string $sessionId): JsonResponse
    {
        try {
            $e = $this->enrollHandler->handle(new EnrollEmployeeCommand($sessionId, $r->input('employee_id')));

            return response()->json(['id' => $e->getId()->value], 201);
        } catch (\Exception $ex) {
    ...
    {
        try {
            $this->cancelHandler->handle(new CancelEnrollmentCommand($id));
    ...
    {
        try {
            $this->attendanceHandler->handle(new RecordAttendanceCommand($id, $r->input('attendance', [])));
    ...
    {
        try {
            $this->completeHandler->handle(new CompleteEnrollmentCommand($id));
