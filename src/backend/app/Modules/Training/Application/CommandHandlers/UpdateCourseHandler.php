...
{
    public function __construct(private readonly TrainingCourseRepositoryInterface $repo) {}

    public function handle(UpdateCourseCommand $cmd): void
    {
        $c = $this->repo->findById(TrainingCourseId::fromString($cmd->id)) ?? throw new TrainingCourseNotFoundException($cmd->id);
        $c->update($cmd->code, $cmd->name, $cmd->description, $cmd->category, $cmd->defaultDurationHours, $cmd->maxParticipants);
        $this->repo->save($c);
    }
}
