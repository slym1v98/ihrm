...
class CreateCourseHandler
{
    ...
    public function handle(CreateCourseCommand $cmd): TrainingCourse
    {
        $c = TrainingCourse::create(TrainingCourseId::generate(), $cmd->code, $cmd->name, $cmd->description, $cmd->category, $cmd->defaultDurationHours, $cmd->maxParticipants);
        $this->repo->save($c);
