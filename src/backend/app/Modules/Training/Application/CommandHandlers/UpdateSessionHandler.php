...
class UpdateSessionHandler
{
    ...
    public function handle(UpdateSessionCommand $cmd): void
    {
        $s = $this->repo->findById(TrainingSessionId::fromString($cmd->id)) ?? throw new TrainingSessionNotFoundException($cmd->id);
        $s->update($cmd->code, $cmd->name, new \DateTimeImmutable($cmd->startDate), new \DateTimeImmutable($cmd->endDate), $cmd->location, $cmd->instructor, $cmd->maxParticipants);
        $this->repo->save($s);
    }
}
