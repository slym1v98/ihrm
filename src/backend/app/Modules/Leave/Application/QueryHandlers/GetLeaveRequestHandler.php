...
class GetLeaveRequestHandler
{
    public function __construct(private LeaveRequestRepositoryInterface $requests) {}

    public function handle(GetLeaveRequestQuery $query): mixed
    {
        return $this->requests->findById(new LeaveRequestId($query->id)) ?? throw new LeaveRequestNotFoundException('Leave request not found');
    }
}
