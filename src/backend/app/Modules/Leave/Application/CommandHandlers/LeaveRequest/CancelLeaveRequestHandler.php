...
{
    public function __construct(private LeaveRequestRepositoryInterface $requests, private LeaveTypeRepositoryInterface $types, private LeaveBalanceRepositoryInterface $balances) {}

    public function handle(CancelLeaveRequestCommand $command): LeaveRequest
    {
        $request = $this->requests->findById(new LeaveRequestId($command->id));
        if (! $request) {
            throw new LeaveRequestNotFoundException('Leave request not found');
        } $wasApproved = $request->status() === LeaveStatus::APPROVED;
        Event::dispatch($request->cancel($command->cancelledBy));
        if ($wasApproved && $this->types->findById($request->leaveTypeId())?->isBalanceTracked()) {
            $balance = $this->balances->findByEmployeeTypeYear($request->employeeId(), $request->leaveTypeId(), (int) $request->period()->startAt()->format('Y'));
            if ($balance) {
                Event::dispatch($balance->restore($request->period()->durationMinutes()));
