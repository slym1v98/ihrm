...
class ApproveLeaveRequestHandler
{
    ...
    public function handle(ApproveLeaveRequestCommand $command): LeaveRequest
    {
        ...
        } $type = $this->types->findById($request->leaveTypeId());
        $balanceBefore = null;
        if ($type?->isBalanceTracked()) {
            ...
            Event::dispatch($balance->deduct($request->period()->durationMinutes()));
            $this->balances->save($balance);
        } Event::dispatch($request->approve($command->approvedBy, $balanceBefore));
