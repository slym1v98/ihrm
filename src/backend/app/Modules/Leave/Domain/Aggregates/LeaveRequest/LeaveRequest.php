...
class LeaveRequest
{
    ...
    public function id(): LeaveRequestId
    {
        return $this->id;
    }
    ...
    public function leaveTypeId(): LeaveTypeId
    {
    ...
    public function approve(string $userId, ?int $balanceBefore = null): LeaveRequestApproved
    {
        $this->requirePending();
        $this->status = LeaveStatus::APPROVED;
        $this->approvedBy = $userId;
        $this->approvedAt = CarbonImmutable::now();
        $this->balanceBefore = $balanceBefore;

        return new LeaveRequestApproved(['leave_request_id' => $this->id->value(), 'approved_by' => $userId]);
    }

    public function reject(string $userId, string $reason): LeaveRequestRejected
    {
        $this->requirePending();
        $this->status = LeaveStatus::REJECTED;
        $this->rejectedReason = $reason;

        return new LeaveRequestRejected(['leave_request_id' => $this->id->value(), 'rejected_by' => $userId, 'reason' => $reason]);
    }

    public function cancel(string $userId): LeaveRequestCancelled
    {
        if (! in_array($this->status, [LeaveStatus::PENDING, LeaveStatus::APPROVED], true)) {
            throw new InvalidLeaveStatusTransitionException('Only pending or approved leave can be cancelled');
        } $this->status = LeaveStatus::CANCELLED;

        return new LeaveRequestCancelled(['leave_request_id' => $this->id->value(), 'cancelled_by' => $userId]);
    }

    public function submittedEvent(): LeaveRequestSubmitted
    {
        return new LeaveRequestSubmitted(['leave_request_id' => $this->id->value(), 'employee_id' => $this->employeeId]);
    }

    private function requirePending(): void
    {
        if ($this->status !== LeaveStatus::PENDING) {
            throw new InvalidLeaveStatusTransitionException('Leave request must be pending');
        }
    }
}
