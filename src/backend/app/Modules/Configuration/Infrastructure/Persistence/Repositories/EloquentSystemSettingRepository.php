...
class EloquentSystemSettingRepository implements SystemSettingRepositoryInterface
{
    ...
    {
        return SystemSettingModel::orderBy('key')->paginate($perPage);
    }

    public function findByKey(string $key): ?SystemSettingModel
    {
        return SystemSettingModel::where('key', $key)->first();
    }

    public function save(array $attributes): SystemSettingModel
    {
        return SystemSettingModel::updateOrCreate(['key' => $attributes['key']], $attributes);
    }
}
