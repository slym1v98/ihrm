...
final class ResolverRegistry
{
    ...
    public function register(AssigneeResolver $resolver): void { $this->resolvers[$resolver->key()] = $resolver; }
    public function get(string $key): AssigneeResolver {
        if (!isset($this->resolvers[$key])) throw new WorkflowResolverNotFoundException("Không tìm thấy assignee resolver: {$key}");
