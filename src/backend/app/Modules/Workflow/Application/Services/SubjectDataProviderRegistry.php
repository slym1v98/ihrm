...
final class SubjectDataProviderRegistry
{
    ...
    public function register(SubjectDataProvider $p): void
    {
        $this->providers[$p->subjectType()] = $p;
    }

    public function get(string $t): SubjectDataProvider
    {
        if (! isset($this->providers[$t])) {
            throw new WorkflowSubjectProviderNotFoundException("Không tìm thấy subject provider cho loại {$t}");
        }
